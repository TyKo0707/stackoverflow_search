...
def search_results1(search_string, num_results):
    ...
    if len(tags) != 0:
        ...
        mask = [True if len(tags.intersection(set(preprocessed_data1.iloc[i].tags))) >= 1 else False
                for i in range(preprocessed_data1.shape[0])]
        data_new = preprocessed_data1[mask]
        ...
        for i, j in cosine_similarities.nlargest(int(num_results)).iteritems():
            output = preprocessed_data1.iloc[i].question_content
            temp = {
        ...
        cosine_similarities = pd.Series(cosine_similarity(search_vect, all_title_embeddings)[0])
        cosine_similarities = cosine_similarities.add(
            (0.4 * preprocessed_data1.overall_scores) + (0.1 * preprocessed_data1.sentiment_polarity) +
...
def search_results2(search_string, num_results):
    # preprocessing the input search string
    search_string = preprocess_text(search_string)
    search_vect = np.array([question_to_vec(search_string, w2v_model)])
...
t0 = time.time()
res1 = search_results1('error_log per virtual host one linux server running apache php 5 multiple virtual hosts '
                       'separate log files seem separate php virtual hosts set apache php log easiest way would', 5)
t1 = time.time()

# t2 = time.time()
# res2 = search_results2('error_log per virtual host one linux server running apache php 5 multiple virtual hosts '
#                        'separate log files seem separate php virtual hosts set apache php log easiest way would', 5)
# t3 = time.time()

with open('results.txt', 'a') as file:
    file.write(f'\nResult of not modified function: {str(t1 - t0)}\n')
    # file.write(f'Result of modified function: {str(t3 - t2)}\n')
